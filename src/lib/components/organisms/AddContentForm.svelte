...
<form>
  ...
  />
  <textarea
    placeholder="Something interesting to read..."
    type="text"
    aria-invalid={errors.content && true}
  ...
  {#if loading}
    <Loader />
  {:else}
    <PreviewContent {originalString} />
  {/if}
</form>

<FloatingRoundButton onClick={handleSubmit}>A</FloatingRoundButton>
